...
class ViewQuest {
    ...
    ViewQuest() {
        ...
        questNo.setFont(new Font("Constantia", Font.BOLD, 18));

        quest_Number = new JLabel();
        quest_Number.setForeground(new Color(163, 175, 39));
        quest_Number.setFont(new Font("Constantia", Font.BOLD, 18));
        ;
        ...
        try {
            ResultSet rs = dbConnection.getStatement().executeQuery("SELECT * FROM `reservations` WHERE `Res`='" + viewColumnData + "' ORDER BY `Res`");
        ...
        closeButton.addActionListener(e -> ViewQuest.dispose());
